...
echo "== campaign-tagged requests (blob7, v3+) =="
# Grouped on the whole query string rather than the extracted source: AE SQL
# has no extract()/regex function (its parser reads extract as the date one),
# and the full string is more useful anyway -- source, medium and campaign
# together rather than one field.
q "SELECT blob7 AS query, blob1 AS path, sum(_sample_interval) AS requests FROM $D WHERE timestamp > $SINCE AND blob7 LIKE '%utm_%' GROUP BY blob7, blob1 ORDER BY requests DESC LIMIT 10"
...
q "SELECT blob1 AS path, count(DISTINCT blob7) AS distinct_queries, sum(_sample_interval) AS requests FROM $D WHERE timestamp > $SINCE AND blob7 != '' GROUP BY blob1 HAVING distinct_queries > 5 ORDER BY distinct_queries DESC LIMIT 10"
